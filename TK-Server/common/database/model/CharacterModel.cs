...
namespace common.database.model
{
    public class CharacterModel : IRedisModel
    {
        ...
        public async Task InitAsync(IDatabase db, params string[] args)
        {
            ...
            BigSkills = new bool[12];

            for (var i = 0; i < BigSkills.Length; i++)
                BigSkills[i] = await db.ReadAsync<bool>(_key, $"bigSkill{i + 1}");

            SmallSkills = new int[12];

            for (var i = 0; i < BigSkills.Length; i++)
                SmallSkills[i] = await db.ReadAsync<int>(_key, $"smallSkill{i + 1}");
        ...
        public async Task SaveAsync()
        {
            ...
            for (var i = 0; i < BigSkills.Length; i++)
                await _db.WriteAsync(_key, $"bigSkill{i + 1}", BigSkills[i]);

            for (var i = 0; i < SmallSkills.Length; i++)
                await _db.WriteAsync(_key, $"smallSkill{i + 1}", SmallSkills[i]);
